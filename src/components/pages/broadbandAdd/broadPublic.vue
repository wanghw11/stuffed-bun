<template>
  <div id="broadPublic" :class="{focusState :showFocus}">
      ...
      <div class="content">
        ...
        <div v-if="showProduct && productselect.length>0" style="cursor:pointer;"
          ...
          <table :style="{'background':item.action?'#B0C4DE':'#f7f8fa'}"
                 @click="checkProduct(item,item.onuItems[0],item.iptvItems[0],item.iptvNumItems[0])" class="shangpin">
            <tbody>
            <tr>
              <td>商品名称：</td>
            ...
            <tr>
              <td>光&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;猫：</td>
              <td><input type="hidden" name="onuRadio" :value="item.onuItems[0].policyId"/>{{
                  item.onuItems[0].commName
                }}&nbsp;&nbsp;&nbsp;&nbsp;
              </td>
            </tr>
            <tr>
              ...
              <td>
                <div v-if="item.iptvItems.length>0">
                  <input style="width: 16px;margin-right: 10px;" type="checkbox"
                         @click="changeCheckbox(item.iptvItems[0])" v-model="item.iptvItems[0].action"
                         :value="item.iptvItems[0].policyId">{{ item.iptvItems[0].commName }}&nbsp;&nbsp;&nbsp;&nbsp;
                  <h2>（可选）</h2>
                </div>
                <div v-else>无</div>
              </td>
            </tr>
            <tr v-if="item.iptvTextlist.length>0">
              <td colspan="2">
                <p style="margin: -15px 0 10px calc(25% + 28px); font-size: 14px;">
                  <input type="hidden" name="iptvMonth"
                         :value="item.iptvTextlist[0].policyId"/>{{ item.iptvTextlist[0].commName }}&nbsp;&nbsp;&nbsp;&nbsp;
                  <input v-if="item.iptvNumItems.length>0" type="hidden" name="iptvSumRadio"
                         :value="item.iptvNumItems[0].iptvNumId"/>&nbsp;&nbsp;&nbsp;&nbsp;
                </p>
              </td>
            </tr>
            <tr v-else><div style="padding-bottom: 10px"></div></tr>
            </tbody>
          </table>
        </div>
    ...
    <div class="content_box">
      <div class="content">
        <h1>入网信息</h1>
        <img @click="checkInterImg" v-if="!showInterImg" class="zhankai" src="@/assets/icon/icon1.png" alt="">
        <img @click="checkInterImg" v-else class="zhankai" src="@/assets/icon/icon2.png" alt="">
        <div class="clearfix"></div>
        <table v-if="showInterImg" style="padding: 0 20px">
          <tbody>
          <tr>
            <td>联系电话：</td>
...
export default {
  ...
  methods: {
    ...
    selectShow() {
      ...
      console.log("商品信息", this.productselect)
      this.showUserInfoImg = false
    ...
    checkProduct(data, onuItemsData, iptvData, iptvNumItemsData) {
      this.productselect.map(item => {
        this.$set(item, 'action', false)
        if (item.iptvItems.length > 0 && iptvData) {
          item.iptvItems.map(itemIptv => {
    ...
    changeCheckbox(data) {
      this.productselect.map(item => {
        this.$set(item, 'action', false)
        if (item.iptvItems.length > 0 && data) {
          item.iptvItems.map(itemIptv => {
    ...
    //入网信息图标
    checkInterImg() {
      this.showInterImg = !this.showInterImg
    },
    loadingStart(tips) {
      this.$toast.loading({
        message: tips,
