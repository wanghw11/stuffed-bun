        ...
        <div class="content">
            <div class="cont">
                ...
                <div class="content_items" v-if="displayFlag15">
                    ...
                    <div class="item2">
                        <div><input type="number" v-model='serialNumber' placeholder="请输入联系电话"></div>
                        <!-- <div class="queryPhone" v-if="phoneShow">请输入正确的手机号</div> -->
                    </div>
                </div>
                <!-- 身份证号 -->
...
export default {
    ...
    computed:{
        ...
        WXfrom(){
            if(sessionStorage.getItem('WXfrom')){
                return sessionStorage.getItem('WXfrom')
            }else{
                return ''
            }
        },
    ...
    methods:{
        ...
        getSatff(){
            if(sessionStorage.getItem('WXfrom')){
                ...
                if(this.global.getUrlParam("phone")){
                    this.$set(this.downData, 'phone', this.global.getUrlParam("phone"))
                }
                if(this.global.getUrlParam("userId")){
                    this.$set(this.downData, 'userId', this.global.getUrlParam("userId"))
                }
                if(this.global.getUrlParam("from")){
                    this.$set(this.downData, 'from', this.global.getUrlParam("from"))
                }
                if(this.global.getUrlParam("extenalUserId")){
                    this.$set(this.downData, 'extenalUserId', this.global.getUrlParam("extenalUserId"))
                }
                if(this.global.getUrlParam("merterialId")){
                    this.$set(this.downData, 'merterialId', this.global.getUrlParam("merterialId"))
                }
                if(this.global.getUrlParam("materialId")){
                    this.$set(this.downData, 'materialId', this.global.getUrlParam("materialId"))
                }
                if(this.global.getUrlParam("mTaskId")){
                    this.$set(this.downData, 'mTaskId', this.global.getUrlParam("mTaskId"))
                }
                if(this.global.getUrlParam("appIdentifier")){
                    this.$set(this.downData, 'appIdentifier', this.global.getUrlParam("appIdentifier"))
                }
                if(this.global.getUrlParam("distinguishType")){
                    this.$set(this.downData, 'distinguishType', this.global.getUrlParam("distinguishType"))
                }
                if(this.global.getUrlParam("qywxId")){
                    this.$set(this.downData, 'qywxId', this.global.getUrlParam("qywxId"))
                }
                if(this.global.getUrlParam("mobilePhone")){
                    this.$set(this.downData, 'mobilePhone', this.global.getUrlParam("mobilePhone"))
                }
                if(this.global.getUrlParam("developId")){
                    this.$set(this.downData, 'developId', this.global.getUrlParam("developId"))
                }
                if(this.global.getUrlParam("flagUrl")){
                    this.$set(this.downData, 'flagUrl', this.global.getUrlParam("flagUrl"))
                }
                this.getCanshu()
            }else{
                if(this.token){
            ...
            if(this.displayFlag11){
                    ...
                    if(res.data.msgFlag != '0'){
                        ...
                        api.down(this.downData).then(res2 => {
                            ...
                            if(res2.data.msgFlag == '0'){
                                ...
                                let orderData = {
                                    error:res2.data.resultJson.detail,
                                    successFlag: '0'
                                }
                                this.$store.commit('updataOrderData', orderData)
                                this.$router.push('/orderSuccess')
                ...
                api.down(this.downData).then(res => {
                    ...
                    if(res.data.msgFlag == '0'){
                        ...
                        let orderData = {
                            error:res.data.resultJson.detail,
                            successFlag: '0'
                        }
                        this.$store.commit('updataOrderData', orderData)
                        this.$router.push('/orderSuccess')
